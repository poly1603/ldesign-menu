 ...
 * 扁平化树形数据
 */
export function flattenTree(items, level = 0, parentId, path = []) {
    const result = [];
    let index = 0;
    function traverse(nodes, currentLevel, currentParentId, currentPath = []) {
        nodes.forEach((node) => {
 ...
 * 获取菜单项的所有父级
 */
export function getMenuItemParents(items, id) {
    const parents = [];
 ...
 * 获取菜单项的所有子级 ID
 */
export function getMenuItemChildrenIds(item) {
    const ids = [];
 ...
 * 过滤菜单树
 */
export function filterMenuTree(items, predicate) {
    return items
        .filter((item) => {
        if (item.children) {
 ...
 * 遍历菜单树
 */
export function traverseMenuTree(items, callback, level = 0, parent) {
    items.forEach((item) => {
 ...
 * 映射菜单树
 */
export function mapMenuTree(items, mapper, level = 0) {
 ...
 * 获取菜单树的最大深度
 */
export function getMenuTreeDepth(items) {
    let maxDepth = 0;
 ...
 * 检查菜单项是否有权限
 */
export function hasPermission(item, userPermissions) {
 ...
 * 根据权限过滤菜单
 */
export function filterMenuByPermissions(items, userPermissions) {
